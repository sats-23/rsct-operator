...
.PHONY: test
test: controller-gen manifests generate fmt vet envtest ## Run tests.
...
.PHONY: build
build: controller-gen manifests generate fmt vet ## Build manager binary.
	go build -o bin/manager cmd/main.go

.PHONY: run
run: controller-gen manifests generate fmt vet ## Run a controller from your host.
	go run ./cmd/main.go
...
.PHONY: install
install: controller-gen manifests kustomize ## Install CRDs into the K8s cluster specified in ~/.kube/config.
...
.PHONY: bundle
bundle: controller-gen manifests kustomize operator-sdk ## Generate bundle manifests and metadata, then validate generated files.
